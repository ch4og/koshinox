...
This repository provides a helper script =config=.

Instead of running long =guix time-machine= invocations manually, you can simply do:
...
(Loaded to =$PATH= from =./scripts/config= with direnv)
